...
Original file is located at
    https://colab.research.google.com/drive/1I0_VJfraFkTHgPAfhePHehk1VO8sx3qJ
"""
...
def inicializar_archivo_huespedes():
    # Create the 'data' directory if it doesn't exist
    os.makedirs(os.path.dirname(ARCHIVO_HUESPEDES), exist_ok=True)
    if not os.path.exists(ARCHIVO_HUESPEDES):
        with open(ARCHIVO_HUESPEDES, mode='w', newline='') as archivo:
...
def registrar_huesped():
    ...
    if not es_nombre_valido(apellido):
        errores.append("El apellido debe tener al menos 3 letras y solo contener caracteres alfabéticos.")

    documento = input("Documento de identidad: ").strip()
    if not es_documento_valido(documento):
        errores.append("El documento debe tener entre 3 y 15 dígitos y solo contener números.")

    correo = input("Correo electrónico: ").strip()
    if not es_correo_valido(correo):
        errores.append("El correo debe tener un formato válido (ejemplo: usuario@dominio.com).")

    telefono = input("Teléfono de contacto: ").strip()
    if not es_telefono_valido(telefono):
        errores.append("El número de teléfono debe tener entre 7 y 15 dígitos y solo contener números.")
...
def inicializar_archivo_reservas():
    if not os.path.exists(ARCHIVO_RESERVAS):
        with open(ARCHIVO_RESERVAS, mode='w', newline='') as archivo:
            writer = csv.writer(archivo)
            writer.writerow(["documento_huesped", "nombre", "tipo_habitacion", "numero_habitacion",
                             "fecha_ingreso", "fecha_salida", "noches", "costo"])

def obtener_huesped(documento):
    with open(ARCHIVO_HUESPEDES, mode='r') as archivo:
        reader = csv.DictReader(archivo)
        for fila in reader:
...
def buscar_habitacion_disponible(tipo):
    with open(ARCHIVO_HABITACIONES, mode='r') as archivo:
        reader = csv.DictReader(archivo)
        for fila in reader:
            if fila["tipo"].lower() == tipo.lower() and fila["estado"].lower() == "disponible":
                return fila
    return None
...
def realizar_reserva():
    ...
    huesped = obtener_huesped(documento)
    ...
    try:
        ...
        print("❌ Error en formato de fecha o número de noches.")
        return

    habitacion = buscar_habitacion_disponible(tipo)
    if not habitacion:
        print(f"❌ No hay habitaciones {tipo} disponibles.")
        return

    # Calcular costo
    precio_noche = int(habitacion["precio"])
    ...
    with open(ARCHIVO_RESERVAS, mode='a', newline='') as archivo:
        ...
        writer.writerow([
            documento,
            f"{huesped['nombre']} {huesped['apellido']}",
            tipo,
            habitacion["numero"],
            fecha_ingreso_str,
    ...
    # Actualizar habitación como reservada
    actualizar_estado_habitacion(habitacion["numero"], "Reservada")
    ...
    print(f"Fecha de ingreso: {fecha_ingreso_str}")
    print(f"Fecha de salida: {fecha_salida.strftime('%Y-%m-%d')}")
    print(f"Noches reservadas: {noches}")
    print(f"Costo total estimado: ${costo_total:,}")

# Inicializar archivo al ejecutar
inicializar_archivo_reservas()
